...
int main() {
    ...
    Texture2D PausedPicture = get_pic(screenWidth / 2, screenHeight / 2, "src/pictures/relaxedcat.png"); // картинка для паузы
    ...
    while (running && !WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_SPACE)) {
            isPaused = !isPaused; // Если нажали пробел для паузы
        }
        if (!isPaused) {
        ...
        if (isPaused) {
            DrawText("PAUSED", screenWidth / 40, screenHeight / 10, screenWidth / 15, DARKGRAY);
            DrawTexture(PausedPicture, screenWidth / 2, screenHeight / 2, WHITE);
        }
        EndDrawing();
    }
